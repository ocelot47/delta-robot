...
def _safe_set_capture(cap, prop: int, value) -> bool:
    try:
...
class CameraHandler:
    ...
    def _open_capture(self, index: int):
        ...
        for backend in backends:
            ...
            with _quiet_stderr():
                _safe_set_capture(cap, cv2.CAP_PROP_FRAME_WIDTH, CAMERA_WIDTH)
                _safe_set_capture(cap, cv2.CAP_PROP_FRAME_HEIGHT, CAMERA_HEIGHT)
                _safe_set_capture(cap, cv2.CAP_PROP_BUFFERSIZE, 1)
